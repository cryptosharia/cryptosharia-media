<script lang="ts">
    ...
    function buildHref(targetPage: number) { const query = new URLSearchParams(); if (data.status) query.set('status', data.status); if (data.search) query.set('q', data.search); if (data.sort !== 'latest') query.set('sort', data.sort); if (targetPage > 1) query.set('page', String(targetPage)); return `/screening${query.size ? `?${query}` : ''}`; }
    function canonicalPath() { const query = new URLSearchParams(); if (data.status) query.set('status', data.status); if (data.sort !== 'latest') query.set('sort', data.sort); if (data.pagination.page > 1) query.set('page', String(data.pagination.page)); return `/screening${query.size ? `?${query}` : ''}`; }
    function filterHref(status: string) { const query = new URLSearchParams(); if (status) query.set('status', status); if (data.search) query.set('q', data.search); if (data.sort !== 'latest') query.set('sort', data.sort); return `/screening${query.size ? `?${query}` : ''}`; }
</script>
...
<main id="main-content" class="site-main">
    ...
    <section class="container screening-discovery" aria-label="Screener coin">
        <form class="screening-search" method="GET" action="/screening" role="search">
            ...
            {#if data.sort !== 'latest'}<input type="hidden" name="sort" value={data.sort} />{/if}
        ...
        <div class="toolbar-meta">
            <form class="sort-control" method="GET" action="/screening">
                ...
                <label for="screening-sort">Urutkan</label><select id="screening-sort" name="sort" value={data.sort} onchange={(event) => event.currentTarget.form?.requestSubmit()}><option value="latest">Terbaru diperbarui</option><option value="az">A–Z</option></select>
            </form>
