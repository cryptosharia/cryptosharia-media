<script lang="ts">
    ...
    const websiteHost = $derived(website ? new URL(website).hostname.replace(/^www\./, '') : null);
    const chartUrl = $derived(data.token.tradingviewSymbol ? `https://www.tradingview.com/chart/?symbol=${encodeURIComponent(data.token.tradingviewSymbol)}` : null);
    ...
    function formatPercent(value: number | null | undefined) { if (typeof value !== 'number' || !Number.isFinite(value)) return '—'; return `${value.toFixed(2)}%`; }
...
<main id="main-content" class="site-main">
    ...
    <section class="container quote-strip" aria-label="Konteks pasar">
        {#if data.quote}
            ...
            <div><span>Dominasi pasar</span><strong>{formatPercent(data.quote.marketCapDominance)}</strong></div>
            <div><span>Supply beredar</span><strong title={String(data.quote.circulatingSupply)}>{formatSupply(data.quote.circulatingSupply)}</strong></div>
            <div><span>Supply maksimum</span><strong title={data.quote.maxSupply === null ? 'Tidak terbatas atau tidak tersedia' : String(data.quote.maxSupply)}>{formatSupply(data.quote.maxSupply, data.quote.infiniteSupply)}</strong></div>
    ...
    {#if chartUrl || website}
        <div class="container market-actions" aria-label="Tautan token">
            {#if chartUrl}<a class="button button-secondary" href={chartUrl} target="_blank" rel="noopener noreferrer">Lihat Grafik <span aria-hidden="true">↗</span></a>{/if}
            {#if website}<a class="website-link" href={website} target="_blank" rel="noopener noreferrer">Website {websiteHost} <span aria-hidden="true">↗</span></a>{/if}
        </div>
    {/if}
...
<style>
 ...
 .detail-outro { max-width:1200px;padding-top:36px;padding-bottom:104px; } .website-link { display:inline-flex;gap:8px;align-items:center;color:var(--text);border:1px solid var(--border-control);border-radius:7px; } .website-link:hover { border-color:var(--accent-text); } .screening-note { display:grid;max-width:760px;grid-template-columns:110px minmax(0,1fr);gap:20px;margin-top:36px;padding:20px 0 0 18px;color:var(--muted);border-top:1px solid var(--border);border-left:2px solid var(--accent);font-size:.9rem;line-height:1.65; } .screening-note p { margin:0;color:var(--text);font-size:.7rem;font-weight:700;letter-spacing:.14em;text-transform:uppercase; }
 :global(.screening-prose h1),:global(.screening-prose h2),:global(.screening-prose h3) { scroll-margin-top:100px; } :global(.screening-prose h2) { position:relative;margin-top:60px;padding:18px 20px;border:1px solid var(--border);border-left:3px solid var(--accent);border-radius:10px;background:var(--surface-muted);font-size:clamp(1.5rem,2.5vw,1.8rem);line-height:1.2;letter-spacing:-.035em; } :global(.screening-prose h3) { margin-top:38px;font-size:1.14rem;line-height:1.3; } :global(.screening-prose p),:global(.screening-prose li) { font-size:1rem;line-height:1.75; } :global(.screening-prose li+li) { margin-top:7px; } :global(.screening-prose ul),:global(.screening-prose ol) { padding-left:1.25rem; } :global(.screening-prose blockquote) { margin-block:28px;padding:16px 18px;border-left:3px solid var(--accent);border-radius:0;background:color-mix(in srgb,var(--surface-muted) 60%,transparent); } :global(.screening-prose table) { display:table;width:min(100%,560px);margin:28px 0;table-layout:fixed;overflow:hidden;border:1px solid var(--border);border-radius:12px;border-collapse:separate;border-spacing:0;background:var(--surface);box-shadow:var(--shadow-sm); } :global(.screening-prose th),:global(.screening-prose td) { width:50%;padding:15px 18px;border:0;border-bottom:1px solid var(--border);text-align:left;vertical-align:middle; } :global(.screening-prose th) { color:var(--text);background:var(--surface-muted);font-size:.76rem;font-weight:800;letter-spacing:.08em;text-transform:uppercase; } :global(.screening-prose tr:last-child td) { border-bottom:0; } :global(.screening-prose td:last-child) { color:var(--success);font-weight:750; }
 @media(max-width:1023px){.analysis-layout{grid-template-columns:minmax(0,760px);justify-content:start}.desktop-toc{display:none}.mobile-toc{display:block;max-width:760px;margin-bottom:34px;border-block:1px solid var(--border)}.mobile-toc summary{padding:13px 0;color:var(--text);font-size:.82rem;font-weight:700;cursor:pointer}.mobile-toc nav{padding:0 0 14px}.mobile-toc[open] summary{color:var(--accent-text)}} @media(max-width:600px){.coin-header{padding-top:48px}.coin-overview{align-items:start;flex-direction:column;gap:22px;margin-top:30px}.coin-identity{gap:14px}.coin-identity img,.coin-logo-fallback{width:48px;height:48px}h1{font-size:clamp(2rem,9vw,2.35rem)}.status-block{display:flex;min-width:0;align-items:center;gap:10px;padding-left:0;border-left:0}.status-block p{display:none}.coin-excerpt{margin-top:18px;font-size:.94rem}.quote-strip{grid-template-columns:repeat(2,minmax(0,1fr));margin-top:38px}.quote-strip>div{padding:15px 10px;border-top:1px solid var(--border)}.quote-strip>div:nth-child(odd){border-left:0}.quote-strip>div:nth-child(even){border-left:1px solid var(--border)}.quote-strip>div:nth-child(-n+2){border-top:0}.quote-strip strong{font-size:1rem}.quote-strip span{font-size:.58rem;letter-spacing:.08em}.market-actions .button,.market-actions .website-link{flex:1}.analysis-layout{padding-top:40px}.detail-outro{padding-top:30px;padding-bottom:80px}.screening-note{grid-template-columns:1fr;gap:7px;margin-top:32px;padding-left:15px}:global(.screening-prose h2){margin-top:48px}:global(.screening-prose p),:global(.screening-prose li){font-size:.94rem;line-height:1.72}}
</style>
